  ...
  const selectImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets.length > 0) {
        ...
        setImageUri(file.uri); // ✅ still okay to set for UI purposes
  
        const formData = new FormData();
        const fileName = file.uri.split('/').pop();
  
        formData.append('image', {
          uri: file.uri,  // ✅ use file.uri instead of imageUri
          name: fileName || 'photo.jpg',
          type: file.type || 'image/jpeg',
        } as any);
  
        console.log("formData=========00000==========================", formData?._parts);
  
        const res = await patchUpdateUserProfile(formData).unwrap();
      ...
      <View style={tw`items-center justify-center`}>
        <View style={tw`bg-[#262329] w-[90%] mt-6 rounded-2xl p-[6%]`}>
          ...
          <View style={tw`w-full items-center mt-18`}>
            <TButton
              onPress={() => router.push('/screens/BecomeContributor')}
